...
function rawMaterialName(options) {
  return `Folleto ${options.Papel} ${options.Color} ${options.Impresion} ${options.Tamano} x${options.Cantidad}`
}
    ...
    if (shouldApply) {
      await prisma.productVariantCost.upsert({
        ...
        create: {
          ...
          providerCost,
          markup: MARKUP,
          sourceLabel: 'lista proveedor folletos',
          sourceRef: 'scratch/seed-provider-print-batch.cjs#folletoTables',
          notes: 'Costo proveedor por tanda segun papel, color, impresion, medida y cantidad.',
        },
        update: {
          rawMaterialId: rawMaterial.id,
          providerCost,
